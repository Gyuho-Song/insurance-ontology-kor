...
for i in $(seq 1 900); do
  ...
  grep -q '^TOTAL:' /tmp/u11_p1_build.log 2>/dev/null && [ "$n" -ge 39 ] && break
  sleep 60
done
n=$(ls $V3/*.graph.json 2>/dev/null | wc -l)
echo "build: $n/39, Failed parse: $(grep -c 'Failed to parse entity' /tmp/u11_p1_build.log)"
...
PYTHONPATH=scripts $VENV scripts/resolve_entities.py \
  --input-dir "$V3" --output "$RESOLVED" --merge-report /tmp/u11_p1_merge.json \
  ...
  PYTHONPATH=scripts $VENV scripts/resolve_entities.py --input-dir "$V3" --output "$RESOLVED" \
    --merge-report /tmp/u11_p1_merge.json --glossary backend-app/app/data/glossary.json --tbox tbox.yaml --no-embedding \
    || { echo "FAIL resolve hard"; exit 3; }; }

step "2. validator (R1/R3=0 기대)"
PYTHONPATH=scripts $VENV -m lib.tbox validate --input "$RESOLVED" --profile report > /tmp/u11_p1_val.txt 2>&1
echo "ERROR 총: $(grep -c '^ERROR' /tmp/u11_p1_val.txt)"
grep '^ERROR' /tmp/u11_p1_val.txt | grep -oE '\[R[0-9]\]' | sort | uniq -c
...
step "5. eval v20 (CloudFront, 라벨매칭 eval)"
$VENV -m pip install -q httpx 2>&1 | tail -1
API_URL="https://$CF/v1/chat" ALB_HOST=$CF $VENV scripts/run_evaluation.py --output /tmp/eval_results_v20.json 2>&1 | tail -20
...
PY
echo "═══ U11 P1 완료 — eval v20, auth 복원 ═══"
